...
why I ultimately decided on doing a statically generated site. I'll also outline my struggles to get this
far.
...
I had 3 options to pick from:

1. Use a modern UI framework and keep using PHP as the back-end
2. Switch to using something like Azure Functions
3. Use a project like mdwiki (or my updated mdwiki-ng)
4. Figure out a way of generating the HTML pages and host them on a CDN)

### Giving it a face-lift w/o changing the back-end

The first option appealed to my lazy side. I already know how to make a site dynamic so all I would
have to do was use an existing UI framework for the front-end.

**Pros**:

- I only have to learn 1 thing
- It'll be the quickest to complete

**Cons**:

- It seemed like the easy way out and didn't serve the purpose
- I'd still have to maintain the server and keep the site working as PHP versions are
  released (i.e. fix my site to make it work again after breaking changes)
- As with any dynamic site, I have to keep an eye out for potential security boo-boos
  (my own or others')
...
This function appealed to me as it was closer to option 1 while letting Amazon/Google/Microsoft
worry about the infrastructure. I would still have to learn a UI framework and chop it
up into pieces that can be dynamically created on-the-fly.

My primary concern was billing. I'm not made of money, and if (for whatever reason)
something I wrote on my site got a big Reddit hug, I'd be paying an unknown amount of
money out the nose. Also, cloud services have sooo many services to achieve something
...
1. mdwiki-ng is built using nodejs and breaks too often for my liking
...
Then serendipity struck. I was browsing [r/webdev](https://old.reddit.com/r/webdev)
...
5. (Learned much later) Can even be built directly on GitHub using GitHub actions

**Cons**:

1. Uses nodejs, an ecosystem I wasn't familiar with at all which confused the hell out of me
...
tl;dr: I had to learn a lot!

I learned about the grid system (pre-CSS-grid) and modern UI frameworks. Again on r/webdev
I came across [halfmoon](https://www.gethalfmoon.com/) a front-end framework with dark-mode
built-in. I tried to shoe-horn it into what I was doing. Through no fault of halfmoon's it
wasn't working for me.

I came across [Astro](https://astro.build/) which was in beta at the time, and it appeared
to be super flexible. It was obvious quite quickly that I was suuuper out of my depth.
Too many new concepts to learn and being in beta, I was bombarded with breaking changes.
After a few weeks spent trying to get it to work, I abandoned the attempt (I think it was
#3 at that point).

Some time later, and after much reflection, I decided that I needed to learn a front-end
framework like AngularJS/ReactJS/VueJS (note how they all end in "JS"?). After much hemming
and hawing (and a fair bit of research) I decided to go with ReactJS. AngularJS seemed too
opinionated to me and VueJS was too new and I didn't want to have a repeat of my Astro
experience.
...
strong background of strongly typed languages (C/C++/C#) and who has been bitten by
many logic bugs involving loosely typed languages (PHP/Python/Javascript).

Serendipity struck again and I was made aware of a "game" called StartTrader. It's an API
only game which has no front-end. That seemed perfect for a simulation of a real-world
project for which to use ReactJS. I wouldn't need to bother with the back-end and can put
all my focus on the front-end.
...
2 other factors helped get me across the line with Astro; first, it has completed the transition
from 1.0 to 2.0 (I first tried it during that transition and that may have contributed to my
first failure). Second, I came across Sat Naing's [astro-paper](https://github.com/satnaing/astro-paper)
...
that I can tweak to exactly what I want. Finally, I like the aesthetic.

## Conclusion

So here we are, I now have a shiny (or dark based on your device's settings) new website which
allows me to post articles to using Markdown. It's tweaked just the way I want it and I understand
exactly what's going on.

All it too was perseverance, a willingness to learn and finding out many ways how NOT to do it.
